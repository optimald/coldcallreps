...
import { DEMO_MSG, isDemoEntityId } from '@/lib/demo/brand-demo-data';
...
/** Parse demo-pb-{brandKey}-{n} → brand slug + template index. */
export function parseDemoPlaybookId(id: string): { brandKey: string; index: number } | null {
  const m = id.match(/^demo-pb-(.+)-(\d+)$/);
  if (!m) return null;
  return { brandKey: resolveDemoBrandKey(m[1]) || m[1], index: Number(m[2]) || 1 };
}

function demoPlaybookForm(playbookId: string, brandKeyHint?: string | null): PlaybookForm | null {
  const parsed = parseDemoPlaybookId(playbookId);
  if (!parsed && !isDemoEntityId(playbookId)) return null;
  const brandKey =
    brandKeyHint ||
    parsed?.brandKey ||
    CANONICAL_DEMO_BRANDS[0]?.slug ||
    'demo-meridianops';
  const brand = CANONICAL_DEMO_BRANDS.find((b) => b.slug === brandKey || b.id === brandKey);
  const short = (brand?.name || 'Brand').split(' ')[0] || 'Brand';
  const index = parsed?.index || 1;
  const template =
    index === 2
      ? DEFAULT_PLAYBOOKS.find((p) => p.key === 'gatekeeper') || DEFAULT_PLAYBOOKS[0]
      : DEFAULT_PLAYBOOKS[0];
  return {
    title: index === 2 ? `${short} gatekeeper transfer` : `${short} high-ticket opener`,
...
export default function PlaybookEditorClient({
  ...
  const [listHref, setListHref] = useState(
    brandKeyProp ? brandHref(brandKeyProp, 'playbooks') : '/practice'
  );
  ...
  useEffect(() => {
    ...
    const demoForm = demoPlaybookForm(id, brandKeyProp);
    if (demoForm) {
      const parsed = parseDemoPlaybookId(id);
      const bk = brandKeyProp || parsed?.brandKey;
      if (bk) setListHref(brandHref(bk, 'playbooks'));
      setDemoMode(true);
      hydrate(demoForm);
    ...
    fetch(`/api/playbooks/${id}`)
      ...
      .then((d) => {
        ...
        const brand = d.playbook.brand as
          | { id?: string; slug?: string | null }
          | null
          | undefined;
        if (brandKeyProp) {
          setListHref(brandHref(brandKeyProp, 'playbooks'));
        } else if (brand?.slug || brand?.id || d.playbook.brandId) {
          setListHref(
            brandHref(
              { id: brand?.id || d.playbook.brandId, slug: brand?.slug || '' },
              'playbooks'
            )
          );
        } else {
          setListHref('/practice');
        }
  ...
  if (notFound || loadError) {
    return (
      <main className="app-page">
        ...
        />
        <Link href={listHref} className="soft-link">
          ← Playbooks
        </Link>
      </main>
    );
  ...
  return (
    <main className="app-page app-page--readable">
      <p style={{ marginBottom: '0.75rem' }}>
        <Link href={listHref} className="muted" style={{ fontWeight: 600 }}>
          ← Playbooks
        </Link>
      </p>
