...
export const PLAN = {
  FREE: {
    ...
    features: [
      `${TRIAL_MINUTES}+ free practice minutes to polish and clear the gate`,
      'All practice scenarios + scorecards',
      'Brand deals free for reps — brands fund escrow',
      'Train → prove → apply → get paid (never pay to earn)',
      'Global leaderboard',
  ...
  STARTER: {
    ...
    annualPriceUsd: 67,
    label: 'Starter',
    audience: 'Heavy practice / teams (optional)',
    minutes: Number(process.env.STARTER_MONTHLY_MINUTES || 100),
    features: [
      `${Number(process.env.STARTER_MONTHLY_MINUTES || 100)} practice minutes / mo`,
      'Optional — not required to earn on brand deals',
      'Brand deals stay free for reps',
