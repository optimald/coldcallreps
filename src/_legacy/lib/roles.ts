...
const I = {
  ...
  team: { href: '/team', label: 'Org roster', icon: 'team' as const },
...
export function canManageBrand(
  ...
  if (brandOwnerId && brandOwnerId === profile.id) return true;
  if (opts?.memberUserIds?.includes(profile.id)) return true;
  return false;
}

/**
 * Sync desk gate (owner + demo). Prefer canAccessBrandDeskAsync when brand.id
 * is available so BrandMember viewers/admins are included.
 */
