...
export async function runScoutCampaignPipeline(payload: ScoutCampaignPayload) {
  ...
  try {
    ...
    await prisma.pipelineJob.update({
      ...
      data: {
        savedCount: count,
        errorMessage: count > 0 ? `Enriching 0/${count}` : null,
      },
    });
    const results = [];
    let readyCount = 0;
    for (let i = 0; i < prospectIds.length; i++) {
      ...
      const result = { id, ...(await runPipelineForProspect(id)) };
      results.push(result);
      if ('outreachReady' in result && result.outreachReady) readyCount += 1;
      await prisma.pipelineJob.update({
        where: { id: job.id },
        data: {
          savedCount: count,
          readyCount,
          errorMessage: `Enriching ${i + 1}/${count}`,
        },
      });
    }
    await prisma.pipelineJob.update({
