...
export async function loadAdminPlatformOverview() {
  ...
  const liveBrand = realBrandWhere();
  const liveUser = realUserWhere();
  const liveCampaign = { brand: liveBrand };
  const liveCall = {
    OR: [{ brand: liveBrand }, { brandId: null, user: liveUser }],
  };
  ...
  const [
    ...
    prisma.brand.count({ where: liveBrand }),
    prisma.userProfile.count({ where: liveUser }),
    prisma.campaign.count({
      where: { status: 'OPEN', ...liveCampaign },
    }),
    ...
    prisma.callLog.count({
      where: { createdAt: { gte: startOfDay }, ...liveCall },
    }),
    prisma.callLog.count({
      where: { createdAt: { gte: day30 }, ...liveCall },
    }),
    prisma.callLog.count({
      where: {
        createdAt: { gte: day30 },
    ...
    prisma.appointmentClaim.count({
      where: { createdAt: { gte: day30 }, campaign: liveCampaign },
    }),
    prisma.appointmentClaim.count({
      where: {
        createdAt: { gte: day30 },
    ...
    prisma.brandWallet.aggregate({
      where: { brand: liveBrand },
      _sum: { balanceCents: true },
    }),
    prisma.campaign.aggregate({
      where: liveCampaign,
      _sum: { escrowLockedCents: true },
    }),
    ...
    prisma.callLog.count({
      where: { needsManualReview: true, ...liveCall },
    }),
    prisma.pipelineJob.count({
      where: { status: 'failed', brand: liveBrand },
    }),
    ...
    prisma.auditLog.findMany({
      where: {
        OR: [{ actorId: null }, { actor: liveUser }],
      },
      orderBy: { createdAt: 'desc' },
      take: 12,
  ...
  const [calls, claims] = await Promise.all([
    prisma.callLog.findMany({
      where: {
        ...
        OR: [{ brand: liveBrand }, { brandId: null }],
      },
      orderBy: { createdAt: 'desc' },
      take: 40,
      select: {
    ...
    prisma.appointmentClaim.findMany({
      where: { status: 'FAILED', campaign: { brand: liveBrand } },
      orderBy: { createdAt: 'desc' },
      take: 40,
      select: {
        id: true,
