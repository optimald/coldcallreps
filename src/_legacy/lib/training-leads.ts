...
/** Legacy brand-desk demo slugs (pre-canonical MeridianOps trio). */
export const LEGACY_DEMO_BRAND_SLUGS = [
  'nike',
  'aegis-life',
  'fleet-co',
  'aegis',
...
/** True for platform demo, QA fixture, or legacy sample brand slugs. */
export function isFixtureBrandSlug(slug: string | null | undefined): boolean {
  if (!slug) return false;
  const s = slug.toLowerCase();
  if (s.startsWith('demo-') || s.startsWith('qa-') || s.startsWith('nike-')) {
    return true;
  }
  return (LEGACY_DEMO_BRAND_SLUGS as readonly string[]).includes(s);
}

/** Exclude seed / placeholder / QA sink rows from ops directories and KPIs. */
...
/**
 * Exclude demo / QA / legacy sample brands from live ops directories and KPIs.
 * In-memory admin Demo desk fixtures are separate (never written to Postgres).
 */
  ...
  return {
    NOT: {
      OR: [
        ...
        { slug: { startsWith: 'qa-' } },
        { slug: { startsWith: 'nike-' } },
        { slug: { in: [...LEGACY_DEMO_BRAND_SLUGS] } },
      ],
    },
  };
}
