...
import { suggestDrillFromImprovements } from '@/lib/drills';
...
export default function TrainerView() {
  ...
  const gateHoldIdRef = useRef<string | null>(null);
  ...
  // Bill minutes if the tab closes mid-call before scorecard runs.
  useEffect(() => {
    function onUnload() {
      const hold = gateHoldIdRef.current;
      if (!hold) return;
      const started = callStartedAtRef.current;
      const durationSecs = started ? Math.floor((Date.now() - started) / 1000) : 0;
      const body = JSON.stringify({ gateHoldId: hold, durationSecs });
      if (typeof navigator !== 'undefined' && navigator.sendBeacon) {
        navigator.sendBeacon(
          '/api/trainer/session-abandon',
  ...
  // Hard mid-call meter: extend hold each minute; force-end when balance is gone.
  useEffect(() => {
    if (!realtime.isConnected || !gateHoldId) return;
    const tick = async () => {
      const started = callStartedAtRef.current;
      if (!started || !gateHoldIdRef.current) return;
      const durationSecs = Math.floor((Date.now() - started) / 1000);
      try {
        const res = await fetch('/api/trainer/session-heartbeat', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            gateHoldId: gateHoldIdRef.current,
            durationSecs,
          }),
        });
        ...
        if (data.forceEnd) {
          setError(data.reason || 'Out of practice minutes — ending call.');
          void endAndScoreRef.current();
        }
      } catch {
        /* best-effort */
    ...
    return () => clearInterval(id);
  }, [realtime.isConnected, gateHoldId]);
  ...
  async function startCall() {
    ...
    try {
      ...
      gateHoldIdRef.current = gate.holdId || null;
      if (typeof gate.minutesRemaining === 'number') setMinutesRemaining(gate.minutesRemaining);
    } catch {
    ...
    setCallStartedAt(started);
    callStartedAtRef.current = started;
  ...
  async function endAndScore() {
    ...
    try {
      ...
      setError(e.message);
      setPhaseLabel('Ready');
      // Score failed — still bill elapsed practice so voice COGS aren't free.
      if (gateHoldId) {
        const hold = gateHoldId;
        void fetch('/api/trainer/session-abandon', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ gateHoldId: hold, durationSecs: duration }),
        })
          .then((r) => (r.ok ? r.json() : null))
          .then((d) => {
            if (d && typeof d.minutesRemaining === 'number') {
            ...
            <>
              ...
              {bountiesCleared.length > 0 && (
                <p style={{ color: 'var(--muted)', fontSize: '0.9rem', margin: 0 }}>
                  Practice bounty cleared (ledger only, not cash):{' '}
                  {bountiesCleared.join(', ')}
                </p>
              )}
              <p style={{ margin: '0.35rem 0 0' }}>{scorecard.summary}</p>
              {(() => {
                const drill = suggestDrillFromImprovements(
                  scorecard.feedback?.improvements,
                  focus
                );
                ...
                return (
                  ...
                  >
                    <strong>Next drill:</strong> {drill.label}
                    <p className="muted" style={{ margin: '0.25rem 0 0.55rem', fontSize: '0.9rem' }}>
                      {drill.reason}
                    </p>
                    <button
                      type="button"
                      className="btn"
                      onClick={() => {
                        setFocus(drill.focus as FocusArea);
                        setRecapOpen(false);
                        setPhaseLabel(`Ready · ${FOCUS_LABELS[drill.focus as FocusArea] || drill.label}`);
                      }}
                    >
                      Practice {drill.label}
                    </button>
                  </div>
                );
              })()}
