...
export default function OutboundDialer({
  campaignProspects: initialProspects,
  campaignId = null,
  activeGigs = [],
  pendingApps = [],
  hasAcceptedCampaign = false,
  outsideCallingHours = false,
  callingHoursHint = null,
  initialLeadId = null,
}: {
  ...
  /** True when accepted campaigns exist but all are outside daily calling hours. */
  outsideCallingHours?: boolean;
  callingHoursHint?: string | null;
  initialLeadId?: string | null;
}) {
  const [campaignProspects, setCampaignProspects] = useState(initialProspects);
  const [outsideHours, setOutsideHours] = useState(outsideCallingHours);
  const [hoursHint, setHoursHint] = useState(callingHoursHint);
  ...
  useEffect(() => {
    setCampaignProspects(initialProspects);
  }, [initialProspects]);

  useEffect(() => {
    setOutsideHours(outsideCallingHours);
    setHoursHint(callingHoursHint);
  }, [outsideCallingHours, callingHoursHint]);

  // Refresh dial queue so leads appear/disappear when calling hours open or close.
  useEffect(() => {
    ...
    async function refreshQueue() {
      try {
        const res = await fetch('/api/prospects?dialable=1&limit=6&fields=list');
        ...
        setCampaignProspects(
          rows.map((l: Record<string, unknown>) => ({
            ...
            ownerName: (l.ownerName as string | null) ?? null,
            ownerTitle: (l.ownerTitle as string | null) ?? null,
            city: (l.city as string | null) ?? null,
            status: String(l.status || ''),
            website: (l.website as string | null) ?? null,
            hooksJSON: (l.hooksJSON as string | null) ?? null,
            notes: (l.notes as string | null) ?? null,
            brandName:
              (l as { brand?: { name?: string } }).brand?.name ??
              (l.brandName as string | null) ??
              null,
            brandSlug:
              (l as { brand?: { slug?: string } }).brand?.slug ??
              (l.brandSlug as string | null) ??
              null,
            attemptCount: typeof l.attemptCount === 'number' ? l.attemptCount : undefined,
            nextCallAt:
              l.nextCallAt == null
                ? null
                : typeof l.nextCallAt === 'string'
                  ? l.nextCallAt
                  : new Date(l.nextCallAt as string).toISOString(),
            lastDisposition: (l.lastDisposition as string | null) ?? null,
          }))
        );
        if (typeof data.outsideCallingHours === 'boolean') {
          setOutsideHours(data.outsideCallingHours);
        }
        if (data.callingHoursHint !== undefined) {
          setHoursHint(data.callingHoursHint || null);
        }
      } catch {
    ...
    const id = window.setInterval(() => void refreshQueue(), 60_000);
    return () => {
      cancelled = true;
              ...
              <div className="cc-desk__gate">
                <p className="cc-desk__gate-title">
                  {outsideHours
                    ...
                    : 'Queue empty — nothing dialable'}
                </p>
                <p className="cc-desk__gate-desc">
                  {outsideHours
                    ? hoursHint ||
                      'Your accepted campaigns only release leads during their calling window. The queue refreshes automatically when hours open.'
                    : 'Leads need a phone number and dial-ready status. Brands generate dial-ready contacts on Generate leads; cooling leads return after their nextCallAt window.'}
                </p>
