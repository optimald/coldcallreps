...
export default function AppShell({
  ...
  useEffect(() => {
    ...
    if (!inOwned && ownedBrands[0]) {
      // Prefer oldest owned brand as stable default (list is createdAt desc).
      const fallback = ownedBrands[ownedBrands.length - 1] || ownedBrands[0];
      const fallbackKey = brandPathKey(fallback);
      setSelectedBrand(fallback);
      writeSelectedBrandKey(fallbackKey);
      syncBrandCookie(fallbackKey);
    }
  ...
  useEffect(() => {
    ...
    if (!resolved) return;
    const key = brandPathKey(resolved);
    // Always persist path brand so account-level routes (/recruit, /billing)
    // don't fall back to brands[0] (newest) and wipe live nav counts.
    writeSelectedBrandKey(key);
    syncBrandCookie(key);
    if (selectedBrand && brandPathKey(selectedBrand) === key) {
                  ...
                  <div className="app-brand-controls">
                    ...
                    >
                      ...
                      {switcherBrands.map((b) => {
                        ...
                        const nameClash =
                          switcherBrands.filter(
                            (x) => x.name.trim().toLowerCase() === b.name.trim().toLowerCase()
                          ).length > 1;
                        return (
                          <option key={b.id} value={key}>
                            {nameClash ? `${b.name} (${key})` : b.name}
                          </option>
                        );
                      })}
                      {deskMode === 'live' ? (
