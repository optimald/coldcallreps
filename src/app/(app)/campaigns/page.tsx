  ...
  return (
    <main className="app-page">
      <PageHeader
        eyebrow="Marketplace"
        title="Campaigns"
        description="Post paid outcome gigs for SDRs — qualified leads or booked meetings. You pay per approved result (~20% platform fee); SDRs receive the rest via Stripe Connect."
        actions={
          ...
          <Panel title="Your campaigns" description="Filter by brand.">
            ...
            {campaigns.length === 0 ? (
              ...
              <div className="page-grid page-grid--wide">
                {campaigns.map((c) => (
                  <Link key={c.id} href={`/campaigns/${c.id}`} className="card-tile">
